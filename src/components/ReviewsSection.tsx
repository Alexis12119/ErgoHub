...
const ReviewsSection: React.FC<ReviewsSectionProps> = ({ reviews }) => {
  ...
  const filteredReviews = reviews.filter((review) => {
    ...
    if (activeFilter === "2 Stars") return review.rating === 3;
    if (activeFilter === "1 Star") return review.rating === 3;
    ...
    <div className="space-y-8">
      ...
      {filteredReviews.length > INITIAL_REVIEW_COUNT && (
        <div className="text-center">
          ...
          >
            See More Reviews ({filteredReviews.length - INITIAL_REVIEW_COUNT}{" "}
            remaining)
          </button>
        </div>
      )}
      ...
      {isReviewsModalOpen && (
        <div className="fixed inset-0 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-6xl w-full h-[90vh] shadow-2xl flex flex-col">
            {/* Modal Header */}
            <div className="flex justify-between items-center p-6 border-b border-gray-200 flex-shrink-0">
              <h2 className="text-xl font-bold text-gray-900">
